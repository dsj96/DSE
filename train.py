...
def gen_train_data(final_model2):
    ...
    According the history checkin sequence, contact the user and poi
    LSTM_train_records_output_: is the train input user_POI sequence
    LSTM_train_records_output_: is the target POI sequence
    '''
...
def gen_test_data(final_model_tensor):
    '''
    LSTM_test_records_input_: list[tensor], use the historical sequence concat with the current POI as input to make recommendation.
    LSTM_test_records_output: the ground truth poiID of the recommendation
    LSTM_test_user_list:      test user list
    '''
    train_user_records = generate_user_records(PATH+'train_checkin_file.txt')
    ...
    for userid,input_poi_list,target_poi_list in LSTM_test_records:
        userid_tensor = final_model_tensor[userid]
        input_ = []
        try:
            for poi in input_poi_list:
                input_.append(torch.cat((userid_tensor, final_model_tensor[poi]), 0))
...
if __name__ == "__main__":
    ...
    final_model_tensor = switch_list_to_tensor()
    LSTM_train_records_input, LSTM_train_records_output = gen_train_data(final_model_tensor)
    train = zip(LSTM_train_records_input, LSTM_train_records_output)
    train_ = []
    for pairs in train:
        train_.append((pairs[0].view(-1,1,INPUT_SIZE), pairs[1].view(-1,1,int(INPUT_SIZE/2))))

    LSTM_test_records_input, LSTM_test_target_poi_list, LSTM_test_user_list = gen_test_data(final_model_tensor)
    ...
    for i in range(EPOCHS):
        total_train_loss = []
        rnn.train()
        for step, (b_x, b_y) in enumerate(train_): # (seq_len, 1, emb_dim*2) (seq_len, 1, emb_dim)
    ...
    final_model_array = switch_tensor_to_array(final_model_tensor)

    '''Calculate similarity scores, excluding user type nodes'''
    node_type = get_node_type(PATH+'node_type.txt')
    history = read_history(PATH+'train_checkin_file.txt')
    print('evaluate!')

    accuracy, precision, recall, ndcg, hit_ratio, MAP = evaleate_all_index_LSTM_no_history(norm_candidate_pois_array, LSTM_test_target_poi_list, node_type, final_model_array)
